...
async def evaluate_test_suite(url: str = None):
    ...
    # 이미 생성된 벡터 데이터베이스가 있는지 확인 - 이미 생성되어 있으므로 주석 처리
    # vector_db_exists = VectorDBManager.check_index_exists(index_id="unified_guidelines_index")
    ...
    # 색인에 데이터 포인트가 없는 문제가 발견되어 데이터를 다시 업로드
    print("\n색인에 데이터 포인트 업로드 시도 중...")
